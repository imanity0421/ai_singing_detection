...
/* ---------- Grade palette: A / S / SS / SSS — saturated warm tones ---------- */
const GRADE_CONFIG: Record<
  string,
  {
    /** Gradient start (deeper, more saturated) */
    from: string
    /** Gradient end (brighter) */
    to: string
...
> = {
  ...
  A: {
    from: "#D89820",
    to: "#F5C848",
    glow: "rgba(216,152,32,0.22)",
    subtitle: "出色发挥",
    cheer: "表现得真好，越唱越有味道！",
  },
}

// Score floor = 80 → default grade is S for positive feedback
    ...
    >
      {!showResult || !evaluation ? null : (
        <>
          ...
          {/* ---- Score Hero ---- */}
          <div
            className="flex flex-col items-center px-6 pt-12 pb-5"
            style={{
              background: `radial-gradient(ellipse 72% 52% at 50% 20%, ${grade.glow} 0%, transparent 100%)`,
            }}
          >
            {/* Clean gradient disc — single layer, no busy rings */}
            <div className="relative">
              {/* Soft outer glow */}
              <div
                className="absolute -inset-6 rounded-full"
                style={{
                  background: `radial-gradient(circle, ${grade.glow} 0%, transparent 68%)`,
                  filter: "blur(16px)",
                }}
              />
              {/* Disc */}
              <div
                ...
                style={{
                  width: 120,
                  height: 120,
                  background: `linear-gradient(145deg, ${grade.from}, ${grade.to})`,
                  boxShadow: `0 8px 32px -4px ${grade.glow}, inset 0 1px 2px rgba(255,255,255,0.32)`,
                }}
              >
                <span
                  className="font-black leading-none text-white"
                  style={{
                    fontSize: scoreLabel.length >= 3 ? 30 : scoreLabel.length === 2 ? 38 : 50,
            ...
            {/* Subtitle pill */}
            <span
              className="mt-4 rounded-full px-5 py-1 text-sm font-bold tracking-widest"
              style={{
                backgroundColor: `color-mix(in srgb, ${grade.from} 12%, transparent)`,
                color: grade.from,
              }}
            >
              {grade.subtitle}
            ...
            {/* Cheer text */}
            <p className="mt-2 max-w-[260px] text-center text-base leading-relaxed text-muted-foreground text-balance">
              {grade.cheer}
            </p>
          </div>
